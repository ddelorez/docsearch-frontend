...
| `users_database.yml` | Generated from `ADMIN_*` variables in `.env` (requires a pre-computed Argon2id hash) |
...
ADMIN_PASSWORD_HASH=$argon2id$v=19$m=65536,t=3,p=4$...   # ← replace with your hash
ADMIN_EMAIL=ddelorez@sginterests.com
ADMIN_DISPLAYNAME=Administrator
```

Generate the Argon2id hash with:
...
| `ADMIN_PASSWORD_HASH` | Yes (for file auth) | — | Pre-computed Argon2id hash for the admin user. Generate with `docker run --rm authelia/authelia authelia crypto hash generate argon2 --password '...' --random-salt --iterations 3 --memory 65536 --parallelism 4`. |
...
| `users: non zero value required` | `users_database.yml` missing, empty, or still the repo placeholder | Set `ADMIN_PASSWORD_HASH` in `.env` with a valid Argon2id hash, then run `./scripts/generate-secrets.sh` |
